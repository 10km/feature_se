...
inline double compareV2(const float *f1, const float *f2, double sssum1, double sssum2) {
	double similarity = dot_product<CODE_FLOAT_NUM,TYPE>(f1, f2)/sqrt(sssum1 * sssum2);
	return  (similarity + 1) / 2.0;
}
/* V1版本特征码比对 */
template<dot_product_type TYPE=DOT_TYPE_CONST>
inline double compareV1(const float *f1, const float *f2) {
...
inline double compare(const face_code &f1,const face_code&f2){
	return compareV2<TYPE>(f1.element,f2.element,f1.sum,f2.sum);
}
} /* namespace feature*/
} /* namespace gdface*/
