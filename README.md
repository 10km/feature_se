...
(人脸)特征内存搜索引擎(feature search engine)，提供高速实时的人脸特征相似度比对搜索/排序,支持多线程并行搜索。(C++11实现)

### feature_se

实现在内存中进行高速特征比对的人脸搜索功能。

人脸搜索引擎的核心是一个允许并发读写的自定义哈希表,特征搜索过程是就是对哈希表中的(人脸)特征多线程分段比对排序/合并结果(类似map/reduce)的过程。

当前项目支持CASSDK人脸识别算法，如果需要支持其他人脸识别算法，只需要替换对应的特征比对函数即可,参见:
